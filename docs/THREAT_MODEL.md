...
- The tab-bound token is still present in the spawned process environment and
  valid for the provider process lifetime, so same-user process inspection on
  the remote remains in scope.
...
- Lower permission modes remain available through debug/provider API
  fields, but provider-native prompts are not yet normalized into
  ShellX's permission pill flow.
...
boundary. Last review: 2026-06-05 (multi-provider and Full Auto
documentation sweep). Next: alongside #330 implementation.
