...
//!
//! 1. Resolves xAI access OAuth-first from the installed grok-build login,
//! then from ShellX Vault, then from developer environment overrides. A legacy
//! local password-store fallback is kept only for migration compatibility.
//!
...
//! to the user):
//! - API key missing → asks the user to run `grok login` or add `xai/api-key`
//! in Settings -> Vault.
...
/// 2. **ShellX Vault** — `xai/api-key`. For users
...
/// Read a legacy local password-store entry with PINENTRY disabled so a locked
/// GPG cache fails fast. Returns Some(trimmed_value) on success, None on any
/// failure.
...
/// label only (`"vault"`, `"env"`, `"pass:xai/api-key"`,
/// `"pass:grok/api-key"`, or `"none"`), NEVER the key value itself.
///
...
pub async fn voice_credential_source() -> String {
    ...
    let vault = crate::shellx_vault::shared_backend();
    if let Ok(Some(_)) = vault.compat_get("xai/api-key").await {
