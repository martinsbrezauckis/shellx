...
// for LOCAL grok on the same Windows box (or WSL/SSH running its own
...
fn tab_bound_mcp_token_from_base(base_token: &str, tab_id: &str) -> String {
    use sha2::{Digest, Sha256};
    let mut hasher = Sha256::new();
    hasher.update(b"shellx-mcp-tab-token-v1\0");
    hasher.update(base_token.as_bytes());
    hasher.update(b"\0");
...
fn token_matches_tab(token: &str, base_token: &str, tab_id: &str) -> bool {
    let expected = tab_bound_mcp_token_from_base(base_token, tab_id);
    subtle_eq(token.as_bytes(), expected.as_bytes())
}
...
pub(crate) fn scrub_credentials(value: &mut serde_json::Value) {
    ...
    fn looks_like_protocol_or_tool_identifier(s: &str) -> bool {
        let t = s.trim();
        if t.is_empty()
            || t.len() > 128
            || t.starts_with("Bearer ")
        ...
        let is_secret_tool_identifier = matches!(
            lower.as_str(),
        ...
        let alpha_count = t.chars().filter(|c| c.is_ascii_alphabetic()).count();
        let digit_count = t.chars().filter(|c| c.is_ascii_digit()).count();
        if alpha_count < 3 || digit_count > 4 {
            return false;
        }

        t.split(['_', '-', '/', '.'])
            .filter(|segment| !segment.is_empty())
            .all(|segment| segment.chars().any(|c| c.is_ascii_alphabetic()))
    }

    fn looks_like_build_scratchboard_reference(s: &str) -> bool {
        ...
        if t.is_empty() || t.len() > 180 || t.starts_with("Bearer ") || t.starts_with("Basic ") {
            return false;
        }
        let lower = t.to_ascii_lowercase();
    ...
    fn looks_like_generated_media_path_reference(s: &str) -> bool {
        let mut t = s.trim_matches(|c: char| {
            matches!(
                c,
                '"' | '\'' | '`' | ',' | ';' | '(' | ')' | '[' | ']' | '{' | '}' | '<' | '>'
            )
        ...
        if t.is_empty() || t.starts_with("Bearer ") || t.starts_with("Basic ") {
            return false;
        }
        let lower = t.to_ascii_lowercase();
        if [
        ...
        let has_media_ext = [
            ".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp", ".svg", ".ico", ".mp4", ".webm",
            ".mov", ".m4v", ".mkv",
        ]
        .iter()
    ...
    fn looks_like_filesystem_path_reference(s: &str) -> bool {
        let mut t = s.trim_matches(|c: char| {
            matches!(
                c,
                '"' | '\'' | '`' | ',' | ';' | '(' | ')' | '[' | ']' | '{' | '}' | '<' | '>'
        ...
        for prefix in [
            "path=", "PATH=", "cwd=", "CWD=", "file=", "FILE=", "dir=", "DIR=",
        ] {
            if let Some(rest) = t.strip_prefix(prefix) {
                t = rest;
                break;
        ...
        if t.is_empty() || t.len() > 512 || t.starts_with("Bearer ") || t.starts_with("Basic ") {
            return false;
        }
        let lower = t.to_ascii_lowercase();
        if [
    ...
    fn looks_like_shellx_routing_reference(s: &str) -> bool {
        let t = s.trim_matches(|c: char| {
            matches!(
                c,
                '"' | '\'' | '`' | ',' | ';' | ':' | '(' | ')' | '[' | ']' | '{' | '}' | '<' | '>'
        ...
        if t.len() < 8 || t.len() > 180 || t.starts_with("Bearer ") || t.starts_with("Basic ") {
            return false;
        }
        let lower = t.to_ascii_lowercase();
        if [
        ...
        if !(lower.starts_with("provider-session-")
            || lower.starts_with("audit-")
            || lower.starts_with("tab-")
            || lower.starts_with("conn-"))
        {
            return false;
        }
        t.chars()
    ...
    fn looks_like_credential(s: &str) -> bool {
        ...
        if looks_like_protocol_or_tool_identifier(t) {
            return false;
        }
        if looks_like_build_scratchboard_reference(t) {
            return false;
        }
        if looks_like_generated_media_path_reference(t) {
            return false;
        }
        if looks_like_filesystem_path_reference(t) {
            return false;
        }
        if looks_like_shellx_routing_reference(t) {
            return false;
        }
        ...
        serde_json::Value::Object(map) => {
            for (k, v) in map.iter_mut() {
                    ...
                    && matches!(v, serde_json::Value::String(s) if looks_like_safe_identifier(s))
                {
                    ...
                    && matches!(v, serde_json::Value::Array(arr) if arr.iter().all(|item| matches!(item, serde_json::Value::String(s) if looks_like_safe_identifier(s))))
                {
                    // Same for plural id arrays such as Agent_poll_all's
                    // `subagent_ids`; these are routing handles, not secrets.
                } else {
                    scrub_credentials(v);
                }
            }
...
mod scrub_tests {
    ...
    #[test]
    fn preserves_generated_media_path_output() {
        let path = "IMAGE_PATH=/home/user/.codex/generated_images/019e9789-e342-74a0-bb96-dd9ffde49bf4/ig_00aff7bcb171a9dc016a22b2bf9bb48191aa9d903d1734babe.png";
        let mut v = json!({ "text": path });
        scrub_credentials(&mut v);
        assert_eq!(v["text"], path);
    }

    #[test]
    fn preserves_absolute_media_path_output() {
        let path = "IMAGE_PATH=/home/user/shellx-gpt-image-cross-smoke2.png";
        let mut v = json!({ "text": path });
        scrub_credentials(&mut v);
        assert_eq!(v["text"], path);
    }
    ...
    #[test]
    fn preserves_regular_filesystem_paths_for_debug_controls() {
        let posix = "/home/alice/shellx-surface-console-prototype";
        let windows = "C:\\Users\\FixtureUser\\shellx-surface-console-prototype";
        let mut v = json!({
            "patch": {
                "cwdPicker": {
                    "path": posix,
        ...
        assert_eq!(v["patch"]["cwdPicker"]["path"], posix);
        assert_eq!(v["patch"]["debugInput"]["value"], windows);
        assert_eq!(v["tokenPath"], "***REDACTED***");
    }

    #[test]
    fn preserves_shellx_routing_slugs_in_text() {
        let tab_id = "audit-claude-to-codex-gptimage2-1780659496";
        let run_id = "provider-session-bf11a01c-b9a3-42d3-883d-82937bca4e9b";
        let mut v = json!({ "text": [tab_id, run_id] });
        scrub_credentials(&mut v);
        assert_eq!(v["text"][0], tab_id);
        assert_eq!(v["text"][1], run_id);
    }

    #[test]
    fn still_redacts_generated_media_path_under_sensitive_key() {
        let path = "/home/user/.codex/generated_images/019e9789-e342-74a0-bb96-dd9ffde49bf4/ig_00aff7bcb171a9dc016a22b2bf9bb48191aa9d903d1734babe.png";
        let mut v = json!({ "token": path });
        scrub_credentials(&mut v);
    ...
    #[test]
    fn leaves_acp_protocol_and_tool_identifiers_alone() {
        let mut v = json!({
                ...
                "update": {
                    ...
                    "_meta": {
                        "tools": [
                            ...
                            "grok-shell-host__secret_get",
                            "shellx-host-http__secret_set",
                            "fs_list_dir",
                            "desktop_mouse_drag"
                        ]
                    }
        ...
        assert_eq!(
            v["params"]["update"]["_meta"]["tools"][0],
            "tool_call_delta_chunk"
        );
        assert_eq!(
            v["params"]["update"]["_meta"]["tools"][2],
            "shellx-host-http__Agent"
        );
        assert_eq!(
            v["params"]["update"]["_meta"]["tools"][5],
            "grok-shell-host__secret_get"
        );
        assert_eq!(
            v["params"]["update"]["_meta"]["tools"][6],
            "shellx-host-http__secret_set"
        );
        assert_eq!(
            v["params"]["update"]["_meta"]["tools"][8],
            "desktop_mouse_drag"
        );
    }
...
async fn require_auth(
    ...
    let request_tab_id = mcp_tab_id_from_headers_or_query(&headers, req.uri());
    let ok = match &header_token {
        Some(t) => {
            subtle_eq(t.as_bytes(), cfg.token.as_bytes())
                || request_tab_id
                    .as_deref()
                    .is_some_and(|tab_id| token_matches_tab(t, &cfg.token, tab_id))
        }
        None => false,
    };
    if ok {
...
fn sanitize_mcp_tab_id(value: &str) -> Option<String> {
...
fn mcp_tab_id_from_headers_or_query(headers: &HeaderMap, uri: &axum::http::Uri) -> Option<String> {
    headers
        .get("mcp-tab-id")
        .and_then(|v| v.to_str().ok())
        .and_then(sanitize_mcp_tab_id)
        .or_else(|| {
            uri.query().and_then(|query| {
                query
                    ...
                    .find_map(|(key, value)| {
                        matches!(key, "tabId" | "tab_id" | "tab")
                            .then(|| value)
                            .and_then(decode_query_component)
                            .and_then(|decoded| sanitize_mcp_tab_id(&decoded))
                    })
            })
        })
}
...
async fn mcp_post(
    ...
    let request_tab_id: Option<String> = mcp_tab_id_from_headers_or_query(&headers, &uri);
        ...
        if is_write_class_tool(tname) {
                    ...
                    return json_rpc_error_response(
                        ...
                        format!(
                            "host-MCP: '{}' rejected — missing MCP-Tab-Id header. \
                             ...
                             automatically, or tabId query fallback for providers that \
                             cannot set custom MCP headers; if you see this error, your \
                             provider was not spawned via a ShellX tab.",
                            tname
                        ),
                    );
                }
                ...
                .is_some_and(|token| token_matches_tab(token, &base_token, &tab_id))
            {
                return json_rpc_error_response(
                    ...
                    -32603,
                    format!(
                        "host-MCP: '{}' rejected — write-class tools require a tab-bound ShellX MCP token for tab '{}'",
                        tname, tab_id
                    ),
                );
            }
...
async fn mcp_get() -> Sse<impl futures_util::Stream<Item = Result<Event, Infallible>>> {
    let stream = futures_util::stream::unfold(0_u64, |idx| async move {
        if idx > 0 {
        ...
        Some((Ok(event), idx.saturating_add(1)))
    });
...
mod tests {
    ...
    fn scrub_credentials_redacts_nested_headers() {
        let fake_token = ["xai-supersecret-", "token-12345"].concat();
        let mut v = json!({
    ...
    #[test]
    fn scrub_credentials_preserves_plural_routing_ids() {
        let fake_token = ["xai-abcdefghijkl", "mnopqrstuvwxyz123456"].concat();
        let mut v = json!({
            "subagent_ids": [
                "872283ba-9d58-4f60-8652-d93bc008bc73",
                "759bb620-1148-4f56-85c9-e7b00ac95338"
            ],
            "requestIds": ["req-01", "req-02"],
            "token": fake_token
        });
        scrub_credentials(&mut v);
        assert_eq!(v["subagent_ids"][0], "872283ba-9d58-4f60-8652-d93bc008bc73");
        assert_eq!(v["subagent_ids"][1], "759bb620-1148-4f56-85c9-e7b00ac95338");
        assert_eq!(v["requestIds"][0], "req-01");
        assert_eq!(v["requestIds"][1], "req-02");
        assert_eq!(v["token"], "***REDACTED***");
    }

    #[test]
    fn scrub_credentials_keeps_build_scratchboard_slug_paths() {
        let mut v = json!({
            "content": {
                "text": "C:\\Users\\FixtureUser\\Downloads\\ms-paint-11-api\\build.build-paint-fix-0136.bd5c13c6-1b6.md"
            },
            "chunk": "build-paint-fix-0136.",
            "shortHash": "bd5c13c6-1b6"
        });
        scrub_credentials(&mut v);
        assert_eq!(
            v["content"]["text"],
            "C:\\Users\\FixtureUser\\Downloads\\ms-paint-11-api\\build.build-paint-fix-0136.bd5c13c6-1b6.md"
        );
        assert_eq!(v["chunk"], "build-paint-fix-0136.");
        assert_eq!(v["shortHash"], "bd5c13c6-1b6");
    }

    #[test]
    fn mcp_tab_id_prefers_header_over_query() {
        let mut headers = HeaderMap::new();
        headers.insert("MCP-Tab-Id", "header-tab".parse().unwrap());
        let uri: axum::http::Uri = "/mcp?tabId=query-tab".parse().unwrap();

        assert_eq!(
            mcp_tab_id_from_headers_or_query(&headers, &uri).as_deref(),
            Some("header-tab")
        );
    }

    #[test]
    fn mcp_tab_id_accepts_query_fallback_for_headerless_clients() {
        let headers = HeaderMap::new();
        let uri: axum::http::Uri = "/mcp?tabId=codex-tab-7".parse().unwrap();
    ...
    #[test]
    fn mcp_tab_id_query_fallback_decodes_encoded_tab_id() {
        let headers = HeaderMap::new();
        let uri: axum::http::Uri = "/mcp?tabId=tab%20A%2F%231".parse().unwrap();
    ...
    #[test]
    fn mcp_tab_id_query_fallback_keeps_distinct_special_char_tabs() {
        let headers = HeaderMap::new();
        let uri_a: axum::http::Uri = "/mcp?tabId=tab%20A%2F%231".parse().unwrap();
        let uri_b: axum::http::Uri = "/mcp?tabId=tabA1".parse().unwrap();

        assert_ne!(
            mcp_tab_id_from_headers_or_query(&headers, &uri_a),
            mcp_tab_id_from_headers_or_query(&headers, &uri_b),
        );
    }

    #[test]
    fn tab_bound_mcp_tokens_are_scoped_to_one_tab() {
        let base = "0123456789abcdef0123456789abcdef";
        let tab_a = tab_bound_mcp_token_from_base(base, "tab-a");
        let tab_b = tab_bound_mcp_token_from_base(base, "tab-b");

        assert_ne!(tab_a, base);
        assert_ne!(tab_a, tab_b);
        assert!(token_matches_tab(&tab_a, base, "tab-a"));
