...
use crate::outside_connectors::{
    discord_auth_header, read_vault_value, sanitized_reqwest_error, OutsideConnector,
    OutsideConnectorDispatchMode, OutsideConnectorEventStatus, OutsideConnectorInboundInput,
    OutsideConnectorProvider, OutsideConnectorStore, OutsideConnectorTarget,
};
...
impl RuntimeErrorLogLimiter {
    fn should_log(&mut self, namespace: &str, id: &str, message: &str) -> bool {
        let key = (namespace.to_string(), id.to_string());
        match self.active_errors.get(&key) {
            Some(previous) if previous == message => false,
            _ => {
                self.active_errors.insert(key, message.to_string());
                true
            }
        }
    }

    fn clear(&mut self, namespace: &str, id: &str) {
        self.active_errors
        ...
        for connector in connectors {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    if error_log_limiter.should_log("telegram", &connector.id, &e) {
                        warn!(
...
async fn prepare_external_prompt(
    ...
    Ok(format_external_prompt_message(
        provider,
        conversation_label,
        input,
    ))
...
fn format_external_prompt_message(
    ...
    format!(
        "External {} message for this shellX session.\n\nSender: {}\n{}: {}\n\nMessage:\n{}\n\nReply normally. Your response will be sent back to {}, so keep it concise unless the user explicitly asks for detailed output.",
        provider,
...
fn first_existing_shellx_image_path(text: &str) -> Option<PathBuf> {
    first_existing_shellx_image_path_with_roots(text, &shellx_external_image_roots())
}
...
fn shellx_external_image_roots() -> Vec<PathBuf> {
    ...
    vec![
        ...
        home.join(".grok").join("shellx-preview-screenshots"),
    ]
}
    ...
    loop {
        ...
        let token = match read_vault_value(&token_key).await {
            Ok(token) if !token.trim().is_empty() => {
                error_log_limiter.clear("discord-token", &token_key);
                token
            }
            Ok(_) => {
                update_discord_token_runtime_error(&token_key, Some("discord token is empty"))
                    .await;
                if error_log_limiter.should_log(
                    "discord-token",
                    &token_key,
            ...
            Err(e) => {
                ...
                if error_log_limiter.should_log("discord-token", &token_key, &e) {
                    warn!(
...
async fn dispatch_discord_prompt(
    app: &AppHandle,
    client: &reqwest::Client,
    store: &Arc<OutsideConnectorStore>,
    connector_id: &str,
    ...
    let Some(channel_id) = input
        .conversation_id
        .as_deref()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    else {
        let err = "discord DM channel id is unavailable".to_string();
        let _ = store
            .record_outbound(
                &connector,
                &input,
                OutsideConnectorEventStatus::Error,
                "shellX could not reply because Discord did not provide a DM channel id.",
                Some(err.clone()),
            )
            .await;
        return Err(err);
...
fn split_discord_text(text: &str) -> Vec<String> {
    ...
    let mut current = String::new();
    for ch in text.chars() {
        if current.len() + ch.len_utf8() > 1_900 {
            chunks.push(current);
            current = String::new();
        }
        current.push(ch);
...
mod tests {
    ...
    #[test]
    fn discord_reply_chunks_fit_message_limit() {
        let text = "a".repeat(4_500);
        let chunks = split_discord_text(&text);
        assert_eq!(chunks.concat(), text);
        assert!(chunks.iter().all(|chunk| chunk.len() <= 1_900));
        assert_eq!(split_discord_text("   "), vec!["(empty reply)"]);
    }

    #[test]
    fn external_prompt_names_discord_source() {
        let input = OutsideConnectorInboundInput {
            sender_id: "111222333444555666".into(),
            conversation_id: Some("999888777666555444".into()),
            guild_id: None,
            text: "status please".into(),
        };

        let prompt = format_external_prompt_message("Discord", "DM channel", &input);
    ...
    #[test]
    fn runtime_error_log_limiter_suppresses_repeated_identical_errors() {
        let mut limiter = RuntimeErrorLogLimiter::default();

        assert!(limiter.should_log("telegram", "oconn-1", "vault key missing"));
        assert!(!limiter.should_log("telegram", "oconn-1", "vault key missing"));
        assert!(limiter.should_log("telegram", "oconn-1", "token format invalid"));

        limiter.clear("telegram", "oconn-1");
    ...
    fn telegram_photo_relay_only_allows_shellx_generated_image_paths() {
        ...
        let fake_allowed_dir = root.join("nested").join(".grok").join("sessions");
        fs::create_dir_all(&fake_allowed_dir).expect("fake allowed dir");
