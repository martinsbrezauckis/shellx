...
const SUBAGENT_DENIED_MCP: &[&str] = &[
    "mcp:shellx-host-http/Agent",
    "mcp:shellx-host-http/Agent_kill",
    "mcp:shellx-host-http/send_prompt_to_session",
    "mcp:shellx-host-http/send_prompt_to_provider",
];
...
fn subagent_remote_deny_args() -> String {
    SUBAGENT_DENIED_MCP
        .iter()
...
pub async fn spawn_subagent_with_transport_options(
    ...
    let transport_tab_id = match &transport {
        SubagentTransport::Local { tab_id }
        | SubagentTransport::Wsl { tab_id, .. }
        | SubagentTransport::Ssh { tab_id, .. } => tab_id.as_str(),
    };
    let subagent_tab_id = std::env::var("SHELLX_HOST_MCP_TAB_ID")
        .ok()
    ...
    let task_id_opt: Option<String> = if let Some(reg) = process_registry() {
        ...
        if !subagent_tab_id.is_empty() {
            reg.set_tab_id(&tid, subagent_tab_id.clone()).await;
        }
        Some(tid)
    } else {
        None
    ...
    // status pollers can reach them without locking the registry again.
    {
        ...
        if !subagent_tab_id.is_empty() {
            if let Err(e) =
                crate::host_subagents::set_tab_id(&handle.id.to_string(), &subagent_tab_id)
            {
                tracing::warn!("host_subagents tab ownership mirror failed: {}", e);
            }
        }
    }
...
/// Stamp the shared process-registry row for a spawned subagent with the
/// ShellX tab that requested it. HTTP MCP calls carry the tab id as a
/// request header/query, not an environment variable, so the spawn path
/// cannot infer it on its own.
pub async fn set_tab_id(subagent_id: &str, tab_id: &str) -> Result<bool, String> {
    let id = Uuid::parse_str(subagent_id)
        .map_err(|e| format!("Agent.set_tab_id: bad subagent_id '{}': {}", subagent_id, e))?;
    let h = lookup(id).await?;
    let task_id = {
        let st = h.state.lock().await;
        st.task_id.clone()
    ...
    reg.set_tab_id(&task_id, tab_id.to_string()).await;
    if let Err(e) = crate::host_subagents::set_tab_id(subagent_id, tab_id) {
        tracing::warn!("host_subagents tab ownership mirror failed: {}", e);
    }
    Ok(true)
}
...
pub async fn kill(subagent_id: &str, force: bool) -> Result<Value, String> {
    ...
    if force {
        ...
        if let (Some(reg), Some(task_id)) = (process_registry(), task_id_opt.as_deref()) {
            reg.mark_exited(task_id, None, ProcessStatus::Killed).await;
        }
        return Ok(json!({
            "subagent_id": h.id.to_string(),
            "killed": true,
