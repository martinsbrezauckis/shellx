...
fn expand_vault_placeholders_to_env_refs(s: &str) -> String {
    ...
    while i < bytes.len() {
        if bytes[i..].starts_with(b"$VAULT:") {
            ...
            let ch = s[i..].chars().next().expect("i is inside string");
            out.push(ch);
            i += ch.len_utf8();
        }
    }
        ...
        McpKind::Stdio => {
            ...
            if let Some((cmd, args)) = parts.split_first() {
                let (cmd, args): (String, Vec<String>) = if windows_stdio {
                    ...
                    posix_stdio_command_with_user_path(cmd, args)
                };
...
fn posix_stdio_command_with_user_path(cmd: &str, args: &[&str]) -> (String, Vec<String>) {
    ...
    for arg in args {
        command.push(' ');
        command.push_str(&shell_quote_catalog_arg_for_remote(arg));
    }
...
mod tests {
    ...
    #[test]
    fn vault_placeholder_expansion_preserves_utf8_text() {
        assert_eq!(
...
"#;
    ...
    fn remote_project_config_uses_posix_stdio_commands() {
        ...
        assert!(block.contains("\"-lc\""));
        assert!(block.contains("$HOME/.local/bin"));
        assert!(block.contains("/opt/homebrew/bin"));
        assert!(block.contains("$HOME/.nvm/nvm.sh"));
        assert!(block.contains("exec 'npx' '-y' '@upstash/context7-mcp'"));
