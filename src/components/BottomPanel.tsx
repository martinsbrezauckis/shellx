...
function normalizedSlashCommandName(name: string): string {
  return name.replace(/^\/+/, "");
}
...
export function BottomPanel({
 ...
 // PR/issue list for `#N` autocomplete.
  hashItems = [],
 // Active session slash commands — drives "/" autocomplete in PromptComposer.
  skills = [],
  activeCwd,
  agentId = null,
  onAgentChange,
 ...
 * the workspace and then reuses the normal path-based attach pipeline. */
 ...
 /** Open the session attachment/media board. */
  ...
  hashItems?: HashItem[];
 /** Active session slash commands from `available_commands_update`
 ...
 * when the user starts typing `/` at the prompt's caret. */
 ...
 /** Active tab's cwd — forwarded to BranchPicker for git_branches. */
  ...
  agentProviderScan?: ConnectionProviderScanEntry[];
  scopeProject?: string;
    ...
    <div className="bottom-panel">
      <div className="bottom-tabs">
        ...
        <button
          type="button"
          className="btab btab-action"
          data-debug-id="bottom-action-assets"
        ...
        <button
          ...
          data-debug-id="bottom-tab-stderr"
      ...
      <div className="bottom-body">
        {tab === "Chat" && (
          <PromptComposer
            ...
            agentProviderScan={agentProviderScan}
 ...
 * 🎧 in tab A doesn't bleed into tab B's mic loop. */
 ...
 // can do stop+transcribe+send in one click while the mic is hot.
  ...
  const agentAnchorRef = useRef<HTMLButtonElement | null>(null);
  const [agentPickerCoords, setAgentPickerCoords] = useState<{
    left: number;
    bottom: number;
    width: number;
  } | null>(null);
  const selectedAgentId = normalizeAgentSelection(agentId);
  const agentIsSelected = selectedAgentId !== null;
  const providerAgentSelected = selectedAgentId !== null && selectedAgentId !== "grok";
  const composerReady = connected || agentIsSelected;
  const agentLocked = connected || connectionLocked;
  const agentProviderScanById = useMemo(() => {
    return new Map(agentProviderScan.map((entry) => [entry.providerId, entry]));
  }, [agentProviderScan]);
  const hasAgentProviderScan = agentProviderScan.length > 0;
 ...
 // tab B during tab switches.
  ...
  useEffect(() => {
    if (agentLocked) setAgentPickerOpen(false);
  }, [agentLocked]);

  useLayoutEffect(() => {
    if (!agentPickerOpen) {
      setAgentPickerCoords(null);
      return;
    }
    const anchor = agentAnchorRef.current;
    if (!anchor) return;
    const r = anchor.getBoundingClientRect();
    setAgentPickerCoords({
      left: r.left,
      bottom: window.innerHeight - r.top + 6,
      width: Math.min(380, Math.max(320, window.innerWidth - 24)),
    });
  }, [agentPickerOpen, selectedAgentId, scopeConnection, agentProviderScan]);

  useEffect(() => {
    if (!agentPickerOpen) return;
    const onDoc = (e: MouseEvent) => {
      const t = e.target as HTMLElement;
      if (t.closest('[data-picker-anchor="agent"]')) return;
      if (t.closest("[data-agent-picker-root]")) return;
 ...
 // "conversation" model.
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (!composerReady || isSending) return;
 ...
 // is keyboard-focusable so .click reliably triggers onClick.
  ...
  }, [activeTabId, voiceChatMode, composerReady, isSending]);
 ...
 * doesn't open the popup). */
  ...
  function selectSlashItem(name: string) {
    ...
    const inserted = `/${normalizedSlashCommandName(name)} `;
 ...
 * substring. Cap at 10 visible. */
  const filteredSkills = (() => {
    ...
    return skills
      .map((s) => ({ s, idx: normalizedSlashCommandName(s.name).toLowerCase().indexOf(q) }))
      .filter((x) => x.idx >= 0)
      .sort((a, b) => a.idx - b.idx || normalizedSlashCommandName(a.s.name).localeCompare(normalizedSlashCommandName(b.s.name)))
      .slice(0, 10)
      .map((x) => x.s);
  })();
  ...
  const recomputeSlashCoords = useCallback(() => {
    ...
    const opensBelow = spaceAbove < 180 && spaceBelow > spaceAbove;
    const maxHeight = Math.max(180, Math.min(320, opensBelow ? spaceBelow : spaceAbove));
    setSlashCoords({
      left,
      top: opensBelow ? Math.min(window.innerHeight - 8, rect.bottom + 6) : Math.max(8, rect.top - 6),
      width,
      maxHeight,
  ...
  }, [slashOpen, filteredSkills.length, prompt, recomputeSlashCoords]);
 ...
 * create new; populated = edit existing. */
 ...
 // refetches its list.
  ...
  useEffect(() => {
    if (!debugOpenMenu || debugOpenMenuSeq == null) return;
    ...
    if (debugOpenMenu === "agent") {
      if (!agentLocked && onAgentChange) setAgentPickerOpen(true);
      return;
    }
 ...
 * match between the two; see .composer-mirror in App.css. */}
 ...
 {/* Action row: attach + status + mic + send. */}
        <div className="composer-action">
          <button
            type="button"
            className="attach-btn"
            data-debug-id="composer-attach"
          ...
          <button
            type="button"
            className="attach-btn"
            data-debug-id="composer-screenshot"
 ...
 {/* IDLE/CONNECTED status pill. */}
          <span
            ...
            title={
              providerAgentSelected
                ? `${agentShortLabel(selectedAgentId)} will start in the selected connection when you send.`
                : connected
                  ? "Connected to agent session"
                  : agentIsSelected
                    ? `${agentShortLabel(selectedAgentId)} selected. Send will start this session.`
                    : "Choose an agent before sending"
            }
          >
            <span className={`pd ${composerReady ? "ok" : "idle"}`} />
            <span className="pill-label">
              {providerAgentSelected ? "READY" : connected ? "CONNECTED" : agentIsSelected ? "READY" : "CHOOSE"}
            </span>
          </span>
          {voiceWarning && (
 ...
 * mutually exclusive — only one can be hot at a time. */}
          <MicButton
            ref={micRef}
            mode="talk"
            debugId="composer-talk"
            label="Talk"
            onRecordingChange={setMicRecording}
            disabled={!composerReady || isSending || voiceChatRecording}
            onTranscript={(text) => {
              const ta = taRef.current;
              if (ta) {
 ...
 // conversation feels continuous.
            ...
            disabled={!composerReady || isSending || micRecording}
            onTranscript={(text) => {
 ...
 // user couldn't tap Send first time before any text exists.
            disabled={!composerReady || (!isSending && !anyRecording && prompt.trim() === "" && attachments.length === 0)}
            title={
 ...
 * enforcement; this is the visual reorder only.) */}
        <div className="composer-scope">
          <div style={{ position: "relative" }}>
            ...
            <ConnectionPicker
              ...
              activeId={scopeConnectionId ?? null}
 ...
 // Could also auto-select the saved preset — leave to user.
          ...
          <button
            ref={agentAnchorRef}
            type="button"
            className={`scope-pill agent-pill ${agentLocked ? "locked" : ""}`}
            data-picker-anchor="agent"
            data-debug-id="composer-agent"
            title={agentLocked
              ? "Agent locked after this tab started. Open a new tab to use a different agent."
              : "Choose which agent answers this tab"}
            onClick={() => { if (!agentLocked) setAgentPickerOpen((v) => !v); }}
            disabled={!onAgentChange || agentLocked}
          >
            <span className="sico"><ShellIcon name="message" size={14} /></span>
            <span className="scope-label">Agent: {agentSelectionShortLabel(selectedAgentId)}</span>
            <span className="scaret">
              <ShellIcon name={agentLocked ? "lock" : "chevron-down"} size={12} />
            </span>
          </button>
 ...
 * B work (#207). */}
        ...
        {agentPickerOpen && agentPickerCoords && createPortal(
          <div
            data-agent-picker-root
            className="agent-picker-popover"
            role="menu"
            aria-label="Agent"
            style={{
              position: "fixed",
              left: agentPickerCoords.left,
              bottom: agentPickerCoords.bottom,
              width: agentPickerCoords.width,
            }}
          >
            {selectableAgentOptions.length === 0 && (
              <div className="agent-picker-empty">
                {hasAgentProviderScan
                  ? "No agent CLIs are available in this environment. Refresh the connection scan from Settings."
                  : "Scanning this environment for agent CLIs. Refresh the connection scan from Settings if this does not update."}
              </div>
            )}
            {selectableAgentOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  role="menuitemradio"
                  aria-checked={active}
                  className={`agent-picker-row ${active ? "active" : ""}`}
                  onClick={() => {
                ...
                >
                  <span className="agent-picker-icon">
                    <ShellIcon name={option.kind === "native" ? "message" : "terminal"} size={14} />
                  </span>
                  <span className="agent-picker-text">
                    <span className="agent-picker-name">{option.label}</span>
                    <span className="agent-picker-detail">{detail}</span>
                  </span>
                  {active && <ShellIcon name="check" size={13} />}
                </button>
              );
            })}
          </div>,
 ...
 * above the composer; lists active session commands
 * filtered by the typed query. Arrow keys + Enter
        ...
        {slashOpen && filteredSkills.length > 0 && slashCoords && typeof document !== "undefined" && createPortal(
          <div
            ...
            style={{
              ...
              maxHeight: slashCoords.maxHeight,
            }}
            data-placement={slashCoords.placement}
          >
            ...
            {filteredSkills.map((s, i) => (
              ...
              >
                <span className="slash-pop-command">/{normalizedSlashCommandName(s.name)}</span>
                {s.description && (
...
function stderrLineFromEvent(e: RawEventFrame): string | null {
  if (e.kind === "grok-stderr") {
    return String((e.payload as any)?.line ?? "");
  }
  if (e.kind !== "provider-session-event") return null;
  const payload = e.payload as Record<string, unknown> | null;
  if (!payload || payload.kind !== "raw" || payload.rawType !== "stderr") return null;
  const text = typeof payload.text === "string" ? payload.text : "";
  if (!text.trim()) return null;
  const provider = providerLabelFromPayload(payload.providerId);
  return `${provider}: ${text}`;
}

function providerLabelFromPayload(providerId: unknown): string {
  if (
    providerId === "codex-cli" ||
    providerId === "claude-code" ||
    providerId === "antigravity-cli"
  ) {
    return providerDisplayName(providerId as ProviderId);
  }
