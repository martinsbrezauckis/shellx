/**
 * src/components/PermissionModal.tsx — compatibility-only synchronous gate
 * for defensive `terminal/create` requests from an agent runtime.
 *
 ...
 * runtimes rarely emit concurrent defensive terminal/create requests, so a
 ...
 * a defensive permission path hits `terminal/create`. The channel also carries a
...
export function PermissionModal({
  ...
  const [debugDecision, setDebugDecision] = useState<DebugPermissionDecision | null>(null);
 ...
 * posture: Enter / Space dismisses with deny. */
 ...
 // handle instead of leaking it.
  useEffect(() => {
    if (debugFixture?.surface === "modal") return;
 ...
 // eslint-disable-next-line no-console
  ...
  useEffect(() => {
    if (debugFixture?.surface !== "modal") {
      const ownedRequestId = debugRequestIdRef.current;
      debugRequestIdRef.current = null;
      if (ownedRequestId) {
        setPending((current) => current?.requestId === ownedRequestId ? null : current);
      }
      setDebugDecision(null);
      return;
    }
    debugRequestIdRef.current = debugFixture.requestId;
    setPending({
      requestId: debugFixture.requestId,
      command: debugFixture.command,
      args: [...debugFixture.args],
      cwd: debugFixture.cwd,
      env: [...debugFixture.env],
    });
    setDebugDecision(null);
  }, [debugFixture]);
 ...
 * returns -32001 to grok and the spawn is skipped. */
  const resolve = useCallback((
    allow: boolean,
    event: ShellxUserEventLike | null | undefined,
    source: "backdrop" | "deny" | "allow" | "escape",
  ) => {
    ...
    if (!id) return;
    if (debugFixture) {
      const decision: DebugPermissionDecision = allow ? "allow" : "deny";
      if (
        debugFixture.surface !== "modal"
        || debugFixture.requestId !== id
        || debugFixture.expectedDecision !== decision
        || debugFixture.expectedModalSource !== source
      ) {
        return;
      }
      setPending(null);
      setDebugDecision(decision);
      return;
    }
 ...
 // eslint-disable-next-line no-console
  ...
  }, [debugFixture, pending]);
  useModalFocus(Boolean(pending), dialogRef, (event) => resolve(false, event, "escape"));

  if (!pending) {
    if (!debugFixture || !debugDecision) return null;
    return (
      <div
        className="modal-hint"
        data-shellx-release-control="permission-decision-receipt"
        data-shellx-release-observe="title"
        data-permission-decision={debugDecision}
        title={"Permission decision receipt — " + debugFixture.action + " — " + debugDecision}
      >
        Permission fixture resolved.
      </div>
    );
  }
 ...
 // backdrop and immediately closing the dialog.
    <div
      data-debug-id="surface-components-permissionmodal-1"
      className="modal-backdrop"
      onClick={(event) => resolve(false, event, "backdrop")}
    >
      <div data-debug-id="surface-components-permissionmodal-2"
        ref={dialogRef}
        className="modal"
        role="alertdialog"
        aria-modal="true"
      ...
      >
        <h3 id="permission-modal-title">Agent wants to run a shell command</h3>
        <p style={{ color: "var(--ink-2)", margin: "8px 0 12px" }}>
          The provider requested an explicit decision. Review the command below
          before allowing it.
        </p>
 ...
 {/* Default focus is Deny per the spec. We also place it
          ...
          <button
            ...
            className="pact"
            data-shellx-release-control="permission-modal-deny"
            disabled={Boolean(debugFixture && debugFixture.expectedModalSource !== "deny")}
            onClick={(event: MouseEvent<HTMLButtonElement>) => resolve(false, event, "deny")}
          >
            Deny
          </button>
          <button
            ...
            data-shellx-release-control="permission-modal-allow"
            disabled={Boolean(debugFixture && debugFixture.expectedModalSource !== "allow")}
            onClick={(event: MouseEvent<HTMLButtonElement>) => resolve(true, event, "allow")}
          >
            Allow
          </button>
        </div>
