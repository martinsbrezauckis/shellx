 ...
 * returns -32001 to grok and the spawn is skipped. */
  const resolve = useCallback((allow: boolean, event?: ShellxUserEventLike | null) => {
 ...
 // backdrop and immediately closing the dialog.
    <div className="modal-backdrop" onClick={(event) => resolve(false, event)}>
      <div
        className="modal"
        role="alertdialog"
        aria-modal="true"
 ...
 {/* Default focus is Deny per the spec. We also place it
          ...
          <button
            ...
            type="button"
            className="pact"
            onClick={(event: MouseEvent<HTMLButtonElement>) => resolve(false, event)}
          >
            Deny
          </button>
          <button
            ...
            onClick={(event: MouseEvent<HTMLButtonElement>) => resolve(true, event)}
          >
            Allow
          </button>
        </div>
