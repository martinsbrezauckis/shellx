...
/** Default local folder used by ShellX Browser transfer intents. */
...
function readActiveTab(): SettingsTab {
  try {
    const raw = readMigratedLocalStorage(TAB_KEY, [LEGACY_TAB_KEY]) as SettingsTab | null;
    if (raw && ALL_TABS.includes(raw)) return raw;
  } catch { /* ignore */ }
  return "general";
}
function writeActiveTab(t: SettingsTab): void {
  writeMigratedLocalStorage(TAB_KEY, t, [LEGACY_TAB_KEY]);
}
...
export function normalizeSettings(raw: unknown): SettingsValues {
  ...
  const browserDownloadFolder =
    typeof obj.browserDownloadFolder === "string" ? obj.browserDownloadFolder.trim() : "";
