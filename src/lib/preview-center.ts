...
export interface ShellxPreviewScreenshotInput {
  path: string;
  cwd?: string | null;
  tabId?: string | null;
}
...
const SHELLX_PREVIEW_SCREENSHOT_RE = /^\d{10,}\.png$/i;
...
function shellxPreviewScreenshotName(path: string): string | null {
  const clean = normalizePreviewReference(path).trim().replace(/\\/g, "/").replace(/^\.\//, "");
  if (!clean || clean.includes("/") || previewPathHasParentTraversal(clean)) return null;
  return SHELLX_PREVIEW_SCREENSHOT_RE.test(clean) ? clean : null;
}
...
function userHomeFromCwd(cwd: string): string | null {
  ...
  const wslWinMatch = /^\/mnt\/([a-z])\/Users\/([^/]+)/i.exec(clean);
  if (wslWinMatch?.[1] && wslWinMatch[2]) {
    return `/mnt/${wslWinMatch[1].toLowerCase()}/Users/${wslWinMatch[2]}`;
  }
...
export function resolveShellxPreviewScreenshotPath(
  path: string,
  input: Omit<ShellxPreviewScreenshotInput, "path">,
): string | null {
  const artifact = shellxPreviewScreenshotName(path);
  const tabId = input.tabId?.trim();
  const cwd = input.cwd?.trim().replace(/[\\/]$/, "");
  if (!artifact || !tabId || !cwd) return null;
  ...
  return [
    home,
    ".grok",
    "shellx-preview-screenshots",
    `work-preview-${tabId}-${artifact}`,
  ].join(sep);
}

export function resolvePreviewPath(path: string, cwd?: string | null): string {
  const clean = normalizePreviewReference(path);
  const homeRelative = /^~[\\/](.*)$/.exec(clean);
  if (homeRelative) {
    const home = cwd ? userHomeFromCwd(cwd) : null;
    if (home) {
      const winStyle = /^[A-Za-z]:[\\/]/.test(home) || home.includes("\\");
      const sep = winStyle ? "\\" : "/";
      return `${home.replace(/[\\/]$/, "")}${sep}${(homeRelative[1] ?? "").replace(/^[\\/]+/, "")}`;
    }
    return clean;
  }
