...
export interface BuildRunState {
  ...
  pendingOperatorNotes?: BuildOperatorNote[];
  lastReceiptId?: string | null;
}

export interface BuildOperatorNote {
  noteId: string;
  createdAtMs: number;
  text: string;
}
...
export interface BuildBlockerRecheckResult {
  checked: boolean;
  cleared: boolean;
  message: string;
...
export function isBuildTerminalStatus(status: BuildRunStatus | undefined): boolean {
  return status === "complete" || status === "halted";
}
...
export function shouldQueuePromptAsBuildOperatorNote(
  state: BuildRunState | null | undefined,
): state is BuildRunState {
  return Boolean(state && isBuildVisible(state));
}
...
export function buildStatusLabel(status: BuildRunStatus | undefined): string {
  switch (status) {
    ...
    case "draft": return "Drafting plan";
...
export async function recheckBuildBlocker(tabId: string): Promise<BuildBlockerRecheckResult> {
  return invoke<BuildBlockerRecheckResult>("recheck_build_blocker", { tabId });
}

export async function addBuildOperatorNote(tabId: string, text: string): Promise<BuildOperatorNote> {
  return invoke<BuildOperatorNote>("add_build_operator_note", { tabId, text });
}
