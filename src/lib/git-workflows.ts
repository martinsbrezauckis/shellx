...
export type GitRepoScope = "cwd" | "singleChildRepo";
...
export interface GitSessionStatus {
  ...
  cwd: string;
  repoCwd: string;
  repoScope: GitRepoScope;
  repoCandidates: GitRepoCandidate[];
...
export function gitRepoScopeLabel(status: Pick<GitSessionStatus, "repoScope" | "repoCandidates">): string {
  if (status.repoScope === "singleChildRepo") return "nested repo";
  if (status.repoCandidates.length > 1) return `${status.repoCandidates.length} child repos`;
  return "session cwd";
}
