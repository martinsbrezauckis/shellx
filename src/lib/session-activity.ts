...
function actionsFromCommand(
  ...
  if (first === "git") kind = "git";
  else if (/^(rg|grep|find)\b/.test(first)) kind = "searched";
...
function isShellRedirectionToken(token: string): boolean {
  return /^(?:\d*)[<>]/.test(token) || /^[<>]/.test(token) || token.includes(">&") || token.includes("<&");
}
...
export function filterActivityActions(actions: ActivityAction[], query: string): ActivityAction[] {
  const tokens = query
    .trim()
    .toLowerCase()
    .split(/\s+/)
  ...
  return actions.filter((action) => {
    const haystack = [
      ...
      action.confidence,
      action.source,
      action.sourceType ? `source:${action.sourceType}` : "",
      action.eventType ? `event:${action.eventType}` : "",
