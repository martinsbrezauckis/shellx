...
export type WorkPreviewViewport = "phone" | "tablet" | "desktop";
...
export interface WorkPreviewState {
  ...
  updatedAtMs: number;
  viewportHint: WorkPreviewViewport | null;
  error: string | null;
...
export function workPreviewDefaultViewport(
  state: Pick<WorkPreviewState, "kind" | "viewportHint">,
): WorkPreviewViewport {
  if (state.viewportHint === "phone" || state.viewportHint === "tablet" || state.viewportHint === "desktop") {
    return state.viewportHint;
  }
  return state.kind === "expoWeb" ? "phone" : "desktop";
}
