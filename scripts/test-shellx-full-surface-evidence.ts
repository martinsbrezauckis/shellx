...
type SurfaceSection = typeof SHELLX_FULL_SURFACE_SECTIONS[number];
...
const EVIDENCE_ROOT = process.env.SHELLX_FULL_SURFACE_EVIDENCE_OUT?.trim()
  || join(homedir(), ".shellx", "evidence", "full-surface");
const SCREENSHOT_FULL = process.env.SHELLX_DEBUG_SCREENSHOT_FULL === "1";
const DEFAULT_WAIT_MS = parsePositiveInt(process.env.SHELLX_FULL_SURFACE_TIMEOUT_MS, 60_000);
...
async function postUi(ctx: DebugConnection, body: Json): Promise<void> {
  await api(ctx.base, ctx.token, "POST", "/state/ui", { debugSurface: "app", source: "shellx-full-surface-evidence", ...body });
}
...
async function debugClick(ctx: DebugConnection, selector: string): Promise<void> {
  await postUi(ctx, { debugClick: selector });
}
...
async function main(): Promise<void> {
  ...
  const receipt = {
    schema: "shellx/full-surface-evidence@1",
    generatedAt: new Date().toISOString(),
    status: failed.length === 0 ? "pass" : "fail",
    coverage: {
      everyDeclaredSectionCovered: SHELLX_FULL_SURFACE_SECTIONS.every((section) => rows.some((row) => row.section === section)),
  ...
  if (failed.length > 0) {
    throw new Error(`${failed.length} full surface evidence row(s) failed`);
  }
}
