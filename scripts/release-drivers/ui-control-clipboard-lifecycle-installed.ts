...
import {
  CLIPBOARD_LIFECYCLE_CLEANUPS,
  CLIPBOARD_LIFECYCLE_FIXTURES,
  CLIPBOARD_LIFECYCLE_ORACLES,
  exerciseClipboardLifecycleControl,
  supportsClipboardLifecycleControl,
} from "./ui-control-clipboard-lifecycle";

const manifest: ReleaseSurfaceDriverManifest = {
  schema: RELEASE_SURFACE_DRIVER_MANIFEST_SCHEMA,
  id: "ui-control-clipboard-lifecycle-installed",
  kind: "ui-control",
  ...
  controllerFiles: [
    ...
    "scripts/release-drivers/clipboard-lifecycle-client.ts",
    "scripts/release-drivers/ui-control-clipboard-lifecycle.ts",
    "src/lib/debug-vault-clipboard-fixture.ts",
  ],
  supportedFixtures: [...CLIPBOARD_LIFECYCLE_FIXTURES],
  supportedCleanups: [...CLIPBOARD_LIFECYCLE_CLEANUPS],
  supportedOracles: [...CLIPBOARD_LIFECYCLE_ORACLES],
};
...
async function execute(request: ReleaseSurfaceDriverRequest): Promise<ReleaseSurfaceDriverReport> {
  ...
  for (const assignment of request.assignments) {
    ...
    outcomes.push(await exerciseClipboardLifecycleControl(connection, installedInput, request, assignment));
  }
  return {
