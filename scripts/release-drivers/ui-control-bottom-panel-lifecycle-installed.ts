...
import {
  BOTTOM_PANEL_LIFECYCLE_CLEANUPS,
  BOTTOM_PANEL_LIFECYCLE_FIXTURES,
  BOTTOM_PANEL_LIFECYCLE_ORACLES,
  exerciseBottomPanelLifecycleControl,
  supportsBottomPanelLifecycleControl,
} from "./ui-control-bottom-panel-lifecycle";

const manifest: ReleaseSurfaceDriverManifest = {
  schema: RELEASE_SURFACE_DRIVER_MANIFEST_SCHEMA,
  id: "ui-control-bottom-panel-lifecycle-installed",
  kind: "ui-control",
  ...
  supportedFixtures: [...BOTTOM_PANEL_LIFECYCLE_FIXTURES],
  supportedCleanups: [...BOTTOM_PANEL_LIFECYCLE_CLEANUPS],
  supportedOracles: [...BOTTOM_PANEL_LIFECYCLE_ORACLES],
};
...
async function execute(request: ReleaseSurfaceDriverRequest): Promise<ReleaseSurfaceDriverReport> {
  ...
  for (const assignment of request.assignments) {
    ...
    outcomes.push(await exerciseBottomPanelLifecycleControl(connection, input, request, assignment));
  }
  return {
