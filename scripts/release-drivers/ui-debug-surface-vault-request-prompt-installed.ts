...
const manifest: ReleaseSurfaceDriverManifest = {
  ...
  id: "ui-debug-surface-vault-request-prompt-installed",
...
async function execute(request: ReleaseSurfaceDriverRequest): Promise<ReleaseSurfaceDriverReport> {
  ...
  for (const assignment of request.assignments) {
    if (!supportsVaultRequestPromptSurface(assignment) || assignment.surface.kind !== "ui-debug-surface") {
      throw new Error(`Vault request/prompt debug driver does not support ${assignment.surface.id}`);
    }
