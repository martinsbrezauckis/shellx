...
import {
  NAVIGATION_TAB_CLEANUPS,
  NAVIGATION_TAB_FIXTURES,
  NAVIGATION_TAB_ORACLES,
  exerciseNavigationTabControl,
  supportsNavigationTabControl,
} from "./ui-control-navigation-tabs";

const manifest: ReleaseSurfaceDriverManifest = {
  schema: RELEASE_SURFACE_DRIVER_MANIFEST_SCHEMA,
  id: "ui-control-navigation-tabs-installed",
  kind: "ui-control",
  ...
  supportedFixtures: [...NAVIGATION_TAB_FIXTURES],
  supportedCleanups: [...NAVIGATION_TAB_CLEANUPS],
  supportedOracles: [...NAVIGATION_TAB_ORACLES],
};
...
async function execute(request: ReleaseSurfaceDriverRequest): Promise<ReleaseSurfaceDriverReport> {
  ...
  for (const assignment of request.assignments) {
    if (!supportsNavigationTabControl(assignment)) {
      throw new Error(`navigation-tab driver does not support ${assignment.surface.name}`);
    }
    outcomes.push(await exerciseNavigationTabControl(connection, installedInput, assignment));
  }
  return {
