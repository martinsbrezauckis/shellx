...
import { exerciseTaskEntryControls } from "./ui-task-entry-installed";
import {
  TASK_ENTRY_CLEANUP,
...
const manifest: ReleaseSurfaceDriverManifest = {
  ...
  supportedOracles: [TASK_ENTRY_CONTROL_ORACLE],
};
...
async function execute(request: ReleaseSurfaceDriverRequest): Promise<ReleaseSurfaceDriverReport> {
  ...
  return {
    ...
    outcomes: await exerciseTaskEntryControls(connection, input, request.assignments),
  };
}
