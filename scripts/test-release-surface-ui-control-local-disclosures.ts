...
import { exerciseLocalDisclosureControl } from "./release-drivers/ui-control-local-disclosures";

const root = resolve(import.meta.dirname, "..");
const temp = mkdtempSync(join(tmpdir(), "shellx-ui-local-disclosures-"));
const profileRoot = join(temp, `shellx-final-local-disclosures-${"c".repeat(16)}`);
...
const token = "fixture-ui-local-disclosures-token-0001";
const sessionId = "fixture-local-disclosures-session-0001";
const instanceId = "fixture-local-disclosures-instance-0001";
const sourceCommit = releaseSurfaceFixtureSourceCommit;
const entrypoint = "scripts/release-drivers/ui-control-local-disclosures-installed.ts";
...
try {
  ...
  assert.equal(manifest.id, "ui-control-local-disclosures-installed");
  assert.equal(manifest.invocationTransport, "native-installed-input");
  assert.equal(manifest.supportedFixtures?.length, 8);
  assert.equal(manifest.supportedCleanups?.length, 5);
  assert.equal(manifest.supportedOracles?.length, 11);
  ...
  for (const assignment of request.assignments) {
    outcomes.push(await exerciseLocalDisclosureControl({ base: candidateBase, token }, input, assignment));
  }
  ...
  console.log("Release surface native local-disclosure tests passed (18 controls)");
} finally {
...
function createRequest(candidateBase: string, webdriverBase: string, candidatePort: number): ReleaseSurfaceDriverRequest {
  ...
  const assignments = plan.assignments
    .filter((assignment) => assignment.driverId === "ui-control-local-disclosures-installed")
    .map((assignment) => ({
  ...
  return {
    ...
    driverId: "ui-control-local-disclosures-installed",
...
async function waitForPorts(path: string, child: ChildProcess): Promise<{ candidatePort: number; webdriverPort: number }> {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`local-disclosure fixture exited before startup: ${await streamText(child.stderr)}`);
    }
    try {
  ...
  throw new Error("local-disclosure fixture did not publish its ports");
}
