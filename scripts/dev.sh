#!/usr/bin/env bash
# scripts/dev.sh — launch shellX dev with port overrides that keep
...
#   SHELLX_VITE_PORT=5858 ./scripts/dev.sh
#   SHELLX_VITE_PORT=5858 SHELLX_DEBUG_PORT=5858 \
#     ./scripts/dev.sh
#
...
VITE_PORT="${SHELLX_VITE_PORT:-${GROK_SHELL_VITE_PORT:-5173}}"
DEBUG_PORT="${SHELLX_DEBUG_PORT:-${GROK_SHELL_DEBUG_PORT:-5757}}"
...
export SHELLX_VITE_PORT="$VITE_PORT"
export GROK_SHELL_DEBUG_PORT="$DEBUG_PORT"
export SHELLX_DEBUG_PORT="$DEBUG_PORT"
...
# can opt in via:
#   SHELLX_DISABLE_COMPOSITING=1 ./scripts/dev.sh
#
# X11 backend (agent screenshot mode) — opt in:
#   SHELLX_FORCE_X11=1 ./scripts/dev.sh
...
if [[ "${SHELLX_FORCE_X11:-${GROK_SHELL_FORCE_X11:-0}}" == "1" ]]; then
  : "${GDK_BACKEND:=x11}"
  export GDK_BACKEND
  echo "→ Forced GDK_BACKEND=x11 (agent screenshot mode)"
fi
if [[ "${SHELLX_DISABLE_COMPOSITING:-${GROK_SHELL_DISABLE_COMPOSITING:-0}}" == "1" ]]; then
