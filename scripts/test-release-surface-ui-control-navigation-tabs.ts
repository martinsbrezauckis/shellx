...
import { exerciseNavigationTabControl } from "./release-drivers/ui-control-navigation-tabs";

const root = resolve(import.meta.dirname, "..");
const temp = mkdtempSync(join(tmpdir(), "shellx-ui-navigation-tabs-"));
const profileRoot = join(temp, `shellx-final-navigation-tabs-${"c".repeat(16)}`);
...
const token = "fixture-ui-navigation-tabs-token-0001";
const sessionId = "fixture-navigation-tabs-session-0001";
const instanceId = "fixture-navigation-tabs-instance-0001";
const sourceCommit = releaseSurfaceFixtureSourceCommit;
const entrypoint = "scripts/release-drivers/ui-control-navigation-tabs-installed.ts";
const controllerFiles = [
  "scripts/lib/release-surface-installed-input-client.ts",
  "scripts/lib/release-surface-bounded-observation.ts",
  "scripts/lib/release-surface-macos-native-input.ts",
  "scripts/release-drivers/ui-control-navigation-tabs.ts",
];
...
try {
  ...
  assert.equal(manifest.id, "ui-control-navigation-tabs-installed");
  ...
  for (const assignment of request.assignments) {
    outcomes.push(await exerciseNavigationTabControl({ base: candidateBase, token }, input, assignment));
  }
  ...
  console.log("Release surface native navigation-tab tests passed (14 controls)");
} finally {
...
function createRequest(candidateBase: string, webdriverBase: string, candidatePort: number): ReleaseSurfaceDriverRequest {
  ...
  const assignments = plan.assignments
    .filter((assignment) => assignment.driverId === "ui-control-navigation-tabs-installed")
    .map((assignment) => ({
  ...
  return {
    ...
    driverId: "ui-control-navigation-tabs-installed",
...
async function waitForPorts(path: string, child: ChildProcess): Promise<{ candidatePort: number; webdriverPort: number }> {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`navigation-tab fixture exited before startup: ${await streamText(child.stderr)}`);
    }
    try {
  ...
  throw new Error("navigation-tab fixture did not publish its ports");
}
